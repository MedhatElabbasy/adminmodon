...
export class ShiftService {
  ...
  getAllCompanyTypes() {
    return this.http.get<Lookup[]>(this.url + `api/ShiftType/GetAll`).pipe(
      tap((res) => {
        this.lookups = [...res];
  ...
  add(model: Lookup) {
    this.http
      .post<Lookup>(this.url + `api/ShiftType/Add`, model)
      .subscribe((response) => {
  ...
  update(model: Lookup) {
    this.http
      .post<Lookup>(this.url + `api/ShiftType/Update`, model)
      .subscribe((response) => {
  ...
  delete(id: any) {
    this.http
      .post<Lookup>(this.url + `api/ShiftType/Delete?id=${id}`, null)
      .subscribe((response) => {
        if (response) {
          if (response) {
