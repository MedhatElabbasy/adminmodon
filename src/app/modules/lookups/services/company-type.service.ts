...
export class CompanyTypeService {
  ...
  getAllCompanyTypes() {
    return this.http.get<Lookup[]>(this.url + `api/CompanyType/GetAll`).pipe(
      tap((res) => {
        this.lookups = [...res];
  ...
  add(model: Lookup) {
    this.http
      .post<Lookup>(this.url + `api/CompanyType/Add`, model)
      .subscribe((response) => {
  ...
  update(model: Lookup) {
    this.http
      .post<Lookup>(this.url + `api/CompanyType/Update`, model)
      .subscribe((response) => {
  ...
  delete(id: any) {
    this.http
      .post<Lookup>(this.url + `api/CompanyType/Delete?id=${id}`, null)
      .subscribe((response) => {
        if (response) {
          if (response) {
